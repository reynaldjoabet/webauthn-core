...
/**
  * base64url without padding (RFC 4648 §5), the encoding WebAuthn uses for all binary values
  * exchanged as JSON strings.
  */
object Base64Url {
  ...
  /**
    * Strict decode: the JDK decoder alone tolerates `=` padding, which WebAuthn forbids, so the
    * alphabet/padding check runs first.
    */
